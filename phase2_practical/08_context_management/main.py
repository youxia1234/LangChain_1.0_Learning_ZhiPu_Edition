...
def example_2_summarization_middleware():
    ...
    # 创建带摘要中间件的 Agent
    # 注意：SummarizationMiddleware 需要一个模型实例来生成摘要
...
1. model (必需)
   ...
   - 传入 ChatOpenAI 实例或兼容的模型

2. trigger (必需)
   - 触发摘要的条件
   - 格式: ('tokens', 数值) 或 ('messages', 数值)
   - 示例: trigger=('tokens', 500) 表示超过 500 tokens 触发摘要
   - messages达到触发所需的消息条数，示例：messages = ('messages',10) 表示超过10条消息触发摘要
...
def example_4_manual_trimming():
    ...
    # 只保留最近 5 条消息（使用 token_counter=len 来按消息数计数）
    trimmed = trim_messages(
        messages,
        max_tokens=5,  # 当 token_counter=len 时，max_tokens 表示消息数量
        strategy="last",  # 保留最后的消息
        token_counter=len  # 使用 len 作为计数器，这样 max_tokens 就变成消息数量
    )
