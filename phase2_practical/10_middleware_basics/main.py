...
def example_1_basic_middleware():
    ...
    print(f"Agent: {safe_str(response['messages'][-1].content)}")#safe_str函数有什么作用？
...
class MessageTrimmerMiddleware(AgentMiddleware):
    ...
    def before_model(self, state, runtime):
        ...
        if len(messages) > self.max_messages:
            ...
            # 重要：使用 RemoveMessage 删除所有旧消息，然后添加修剪后的消息
            # 这是 LangGraph 的 add_messages reducer 的要求
            return {
                "messages": [
                    RemoveMessage(id=REMOVE_ALL_MESSAGES),  # 删除所有消息
                    *trimmed_messages,                       # 添加修剪后的消息
                ]
            }
        else:
            return None
...
def example_3_message_trimming():
    ...
    for i in range(6):
        ...
        if len(messages) <= 4:
            print(f"消息列表: {[safe_str(m.content[:15]) for m in messages]}")
...
def example_4_output_validation():
    ...
    print(f"Agent: {safe_str(response['messages'][-1].content[:100])}...")
    ...
    for i in range(4):
        ...
        try:
            ...
            if response.get('messages'):
                print(f"响应: {safe_str(response['messages'][-1].content[:50])}...")
